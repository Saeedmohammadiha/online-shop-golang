...
import (
	...
	"github.com/OnlineShop/internal/pkg/jwt"
)
...
func NewAuthService(u *repositories.UserRepo) AuthService {
	return &Auth{u: *u}
}
...
func (A *Auth) Login(w http.ResponseWriter, r *http.Request) {
	...
	accessToken, err := internaljwt.New().GenerateAccessToken(int(user.ID), user.Roles)
	if err != nil {
		http.Error(w, "can't genrate accssess token", http.StatusBadRequest)
		return
	}
	refreshToken, err := internaljwt.New().GenerateRefreshToken(int(user.ID), user.Roles)
	if err != nil {
